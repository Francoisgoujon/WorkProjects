	...
	public int estime() {
		...
		for (col=0; col<NB_COLONNES; col++) {
			for (lig=0; lig<NB_LIGNES; lig++) {
				...
				// QUESTION 3 : A COMPLETER PAR VOS SOINS
			}
		}
		return (ordi-humain);
	}

	int etiquette(int joueurCourant) {
		if (feuille()) {
			return gagnant();
		}
		else {
			...
			for (int colonne=0; colonne<NB_COLONNES; colonne++) {
				...
				if (fils.peutDeposerPion(colonne)) {
					...
					etiquetteFils=fils.etiquette( adversaire(joueurCourant) );
	...
	public   int meilleurCoupPourOrdinateur() {
		...
		for (int colonne=0; colonne<NB_COLONNES; colonne++) {
			...
			if (fils.peutDeposerPion(colonne)) {
				...
				etiquetteFils=fils.etiquette( HUMAIN );
