...
public class Etat {
	...
	public tripletEtiquetteGagnantPerdant etiquette(int joueurCourant, int profondeur) {
	    int gagnant;
	    tripletEtiquetteGagnantPerdant tripletARetourner;
	    if (feuille()) {
	        gagnant=gagnant();
	        switch (gagnant) {
	        case HUMAIN: tripletARetourner = new tripletEtiquetteGagnantPerdant(gagnant, 0,1);break;
	        case ORDI  : tripletARetourner = new tripletEtiquetteGagnantPerdant(gagnant, 1,0);break;
	        default    : tripletARetourner = new tripletEtiquetteGagnantPerdant(PERSONNE, 0,0);break;
	        }
	        return tripletARetourner;
	    } else {
	    	tripletARetourner = new tripletEtiquetteGagnantPerdant(PERSONNE, 0,0);
	    	if (profondeur==0) {
	    		return tripletARetourner;
	    	} else {
			...
			// ces coups peut nous garantir la victoire.
				...
				if (!filsNul) { // Toutes les colonnes mene l'adversaire a gagner : on est sur de perdre.
	...
	public   int meilleurCoupPourOrdinateur() {
		...
		tripletEtiquetteGagnantPerdant valeurMeilleurCoup=new tripletEtiquetteGagnantPerdant(PERSONNE, -9999999, 9999999);
		tripletEtiquetteGagnantPerdant valeurMoinsPire=new tripletEtiquetteGagnantPerdant(PERSONNE, -9999999, 9999999);
		...
		for (int colonne=0; colonne<NB_COLONNES; colonne++) {
			...
			if (fils.peutDeposerPion(colonne)) {
				...
				if ((etiquetteFils.getEtiquette()==PERSONNE) && (etiquetteFils.meilleur(valeurMeilleurCoup))) {
					valeurMeilleurCoup=etiquetteFils;
					meilleurCoup=colonne;
				} else if (etiquetteFils.meilleur(valeurMoinsPire)){
